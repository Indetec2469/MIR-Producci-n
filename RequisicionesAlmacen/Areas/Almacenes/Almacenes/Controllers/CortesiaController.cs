...
namespace RequisicionesAlmacen.Areas.Almacenes.Almacenes.Controllers
{
    ...
    public class CortesiaController : BaseController<ARtblCortesia, CortesiaViewModel>
    {
        ...
        [JsonException]
        public ContentResult GetProductos()
        {
            var serializer = new JavaScriptSerializer { MaxJsonLength = Int32.MaxValue, RecursionLimit = 100 };

            var result = new ContentResult
            {
                Content = serializer.Serialize(new ProductoService().BuscaActivos()),
        ...
        [JsonException]
        public ContentResult GetCuentasProductos(string almacenId, string productoId)
        {
            var serializer = new JavaScriptSerializer { MaxJsonLength = Int32.MaxValue, RecursionLimit = 100 };

            var result = new ContentResult
            {
                Content = serializer.Serialize(new OrdenCompraService().BuscaComboProductos(almacenId, productoId)),
                ContentType = "application/json"
            };
